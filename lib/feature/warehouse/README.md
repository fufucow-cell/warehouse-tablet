...
在整個 `warehouse` 資料夾中，將所有的 import path 從 `flutter_smart_home_tablet` 替換為目標專案的名稱 `${PROJECT_NAME}`。
...
```bash
# macOS
find lib/feature/warehouse -type f -name "*.dart" -exec sed -i '' 's/flutter_smart_home_tablet/${PROJECT_NAME}/g' {} +
```
