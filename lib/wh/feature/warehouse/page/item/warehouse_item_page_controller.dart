...
class WarehouseItemPageController extends GetxController {
  ...
  void _checkData() {
    ...
    if (_model.allRoomCabinetItems.value != null &&
        _model.allRoomCabinets.value != null) {
      _initData();
    }
  }
  ...
  void _initData() {
    ...
    if (searchCondition != null) {
      ...
      final List<WarehouseNameIdModel> cabinets =
          _model.filterRuleForCabinets.value;
      final matchIndex =
          cabinets.indexWhere((cabinet) => cabinet.id == searchCabinetId);

      if (matchIndex != -1) {
        interactive(EnumWarehouseItemPageInteractive.tapCabinetFilter,
            data: matchIndex);
  ...
  void _genFilterRuleForCabinet() {
    ...
    if (roomIdx == 0) {
      ...
      resultRules.addAll(
        allCabinets
            .where((cabinet) => ((cabinet.id?.isNotEmpty ?? false) &&
                (cabinet.name?.isNotEmpty ?? false)))
            .map((cabinet) =>
                WarehouseNameIdModel(id: cabinet.id, name: cabinet.name)),
      );
    } else {
  ...
  void _genVisibleItemsBySearchCondition() {
    ...
    if (lv1Id.isNotEmpty) {
      ...
      if (lv2Id.isNotEmpty) {
        ...
        if (lv3Id.isNotEmpty) {
          matchItems
              .removeWhere((item) => item.category?.child?.child?.id != lv3Id);
        }
      }
    }
  ...
  // 監聽
  void _addListeners() {
    _model.allRoomCabinetItemsWorker =
        ever(_service.allRoomCabinetItemsRx.rx, (rooms) {
    ...
    _model.searchConditionWorker =
        ever(_service.searchConditionRx.rx, (searchCondition) {
      _model.searchCondition.value = searchCondition;
      if (searchCondition != null) {
        _genVisibleItemsBySearchCondition();
      }
