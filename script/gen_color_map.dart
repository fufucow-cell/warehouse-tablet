...
void main() {
  final colorDataFile = File(
      'lib/feature/warehouse/parent/constant/theme/color_data.dart');
  final outputFile = File(
      'lib/feature/warehouse/parent/constant/theme/color_map.dart');
  ...
  for (final key in allKeys) {
    final gradientMatch =
        RegExp(r'^(.+)_gradient_(\d+)$').firstMatch(key);
  ...
  for (final entry in gradientKeys.entries) {
    entry.value.sort((a, b) {
      final numA = int.tryParse(RegExp(r'_gradient_(\d+)$')
                  .firstMatch(a)
                  ?.group(1) ??
              '0') ??
          0;
      final numB = int.tryParse(RegExp(r'_gradient_(\d+)$')
                  .firstMatch(b)
                  ?.group(1) ??
              '0') ??
          0;
  ...
  final generatedContent =
      _generateColorMap(normalKeys, gradientKeys);
...
String _generateColorMap(
  ...
  // 收集所有枚举值
  final enumEntries =
      <String, bool>{}; // key: enumName, value: isGradient
  ...
  buffer.writeln('');
  buffer
      .writeln('  final _themeUtil = ThemeUtil.instance;');
  buffer.writeln('');
  ...
  buffer
      .writeln('      EnumThemeMode.light => data.light,');
